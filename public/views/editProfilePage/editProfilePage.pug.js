...
export default function editProfilePageComponent(locals) {
  let pug_html = ''; const pug_mixins = {}; let pug_interp; const source = '/static/images/';
  const avatar = source + 'avatar.png';
  const MsgLight = source + 'MsgLight.png';
  const Matches = source + 'CardsDark.png';
  const location = source + 'location.png';
  const settings = source + 'settings.png';
  const horoscopeImg = source + 'horoscope1.png';
  const imgSrc = [source + 'google.svg', source + 'vk.svg', source + 'facebook.svg', source + 'twitter.svg'];
  const navIcons = [source + 'favorite.png', source + 'like.png', source + 'dislike.png'];
  const interests = ['Music', 'Reading a books', 'Watching films', 'Walking with friends'];
